...
from .logic import (
    _log, log_queue, action_done, action_result,
    greeting, find_docker, check_container_status, list_ollama_models,
    write_env, run_docker_compose, install_docker_windows, install_docker_mac, install_docker_linux,
    get_ram_info, get_about_info
)
from .ui import HTML
...
class LucasHandler(http.server.BaseHTTPRequestHandler):
    ...
    def handle(self):
        ...
        try:
            ...
            _log(f"[SERVER] Unexpected error in handle: {type(e).__name__}: {str(e)[:100]}")
    ...
    def do_GET(self):
        ...
        try:
            if p == '/stream':
                ...
                try:
                    while True:
                        if not log_queue.empty():
                            msg = log_queue.get()
                            self.wfile.write(f"data: {json.dumps({'type':'log','line':msg})}\n\n".encode())
                            self.wfile.flush()
                        if action_done.is_set():
                            action_done.clear()
                            self.wfile.write(f"data: {json.dumps({'type':'done','ok':action_result['ok']})}\n\n".encode())
                            self.wfile.flush()
                        time.sleep(0.1)
                except (ConnectionAbortedError, ConnectionResetError, BrokenPipeError):
                    pass
                ...
                _log(f'[SERVER] GET / - serving HTML')
                self.send_response(200)
        ...
        if p == '/api/install':
            ...
            def _run():
                ...
                if ok:
                    ...
                    _log('[LUCAS] 🌐 Ouvrez http://localhost:8080 pour créer votre compte administrateur.')
                else:
            ...
            def _run():
                ...
                if system == 'windows': ok = install_docker_windows()
                elif system == 'darwin': ok = install_docker_mac()
                else: ok = install_docker_linux()
                action_result['ok'] = ok
                action_done.set()
            threading.Thread(target=_run, daemon=True).start()
            self._json({'ok': True})
            ...
            def _parse_ollama_progress(line):
                ...
                match = re.search(r'(\d+)%\s+[▁▂▃▄▅▆▇█▉▊▋▌▍▎▏▕]*\s+(\d+\.?\d*[KMG]B)/(\d+\.?\d*[KMG]B)', line)
                if match:
            ...
            def _run():
                if action == 'install':
                    ...
                    _log(f'[LUCAS] ⏳ Cela peut prendre quelques minutes selon la taille du modèle')
                    try:
                            ...
                            for raw_line in resp:
                                ...
                                if total and completed:
                                    ...
                                    if percent - last_logged_percent >= 5:
                                        ...
                                        _log(f'[LUCAS] 📊 {bar} | {completed//1024//1024}MB/{total//1024//1024}MB')
                        ...
                        _log(f'[LUCAS] 🎉 Installation complète !')
                    ...
                    try:
                        ...
                        def delete_monitor():
                            ...
                            while process.poll() is None:
                                elapsed += 1
                                if elapsed % 2 == 0:
                                    _log(f'[LUCAS] ⏳ Suppression en cours ({elapsed}s)...')
                                time.sleep(1)
                        
                        delete_thread = threading.Thread(target=delete_monitor, daemon=True)
                        delete_thread.start()
                        ...
                        if returncode == 0:
                            ...
                            _log(f'[LUCAS] 🧹 Nettoyage des fichiers en cache...')
                            ...
                            subprocess.run(['docker', 'exec', 'lucas-ollama', 'bash', '-c', clean_cmd], capture_output=True, timeout=10)
                            _log(f'[LUCAS] ✅ Nettoyage terminé')
                            _log(f'[LUCAS] 🎉 Suppression complète !')
                            action_result['ok'] = True
                        else:
                            ...
                            if stderr_lines:
                                for line in stderr_lines[-3:]:
                                    _log(f'[LUCAS] {line}')
                            _log(f'[LUCAS] ⚠️  Le modèle a peut-être déjà été supprimé')
                            action_result['ok'] = True
                    except Exception as e:
