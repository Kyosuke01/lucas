...
def get_ram_info():
    ...
    try:
        ...
        if platform.system().lower() == 'windows':
            # Try wmic first
            try:
                r = subprocess.run(['wmic', 'computersystem', 'get', 'totalphysicalmemory', '/value'], 
                                   capture_output=True, text=True, timeout=3)
                for line in r.stdout.splitlines():
                    if 'TotalPhysicalMemory' in line:
                        v = line.split('=')[1].strip()
                        if v.isdigit(): return round(int(v) / (1024**3), 1)
            except Exception: pass
            
            # Try powershell
            try:
                r = subprocess.run(['powershell', '-command', '(Get-CimInstance Win32_PhysicalMemory | Measure-Object -Property Capacity -Sum).Sum'], 
...
def check_container_status(name):
    ...
    try:
        ...
        if is_win:
            kwargs['creationflags'] = 0x08000000 
        r = subprocess.run(['docker', 'inspect', '-f', '{{.State.Running}}', name], **kwargs)
    ...
    try:
        ...
        if action == 'install' and process.returncode == 0:
            ...
            if target_model:
                ...
                pull_cmd = ['docker', 'exec', 'lucas-ollama', 'ollama', 'pull', target_model]
                ...
                subprocess.run(['docker', 'exec', 'lucas-ollama', 'ollama', 'cp', target_model, tag], capture_output=True)
                if p2.returncode == 0:
                    _log('[LUCAS] ✅ Modèle configuré avec succès !')
                return p2.returncode == 0
            else:
                _log('[LUCAS] ℹ️ Aucun modèle sélectionné — installation des services uniquement.')
                ...
                p2 = subprocess.Popen(cmd2, stdout=subprocess.PIPE, stderr=subprocess.STDOUT, text=True, cwd=str(root))
                for line in p2.stdout:
                    line_stripped = line.rstrip()
                    if line_stripped:
                        _log(line_stripped)
...
def install_docker_windows():
    ...
    try:
        ...
        _log('[ERROR] winget introuvable. Ouvre https://www.docker.com/products/docker-desktop/ pour installer Docker manuellement.')
    ...
    try:
        ...
        for line in process.stdout:
            if line.rstrip(): _log(line.rstrip())
        process.wait()
        if process.returncode == 0:
...
def install_docker_mac():
    ...
    if brew_ok:
        ...
        process = subprocess.Popen(['brew', 'install', '--cask', 'docker'], stdout=subprocess.PIPE, stderr=subprocess.STDOUT, text=True)
        for line in process.stdout:
            if line.rstrip(): _log(line.rstrip())
        process.wait()
        if process.returncode == 0:
...
def install_docker_linux():
    ...
    try:
        process = subprocess.Popen(['bash', '-c', 'curl -fsSL https://get.docker.com | sudo sh'], stdout=subprocess.PIPE, stderr=subprocess.STDOUT, text=True)
        for line in process.stdout:
            if line.rstrip(): _log(line.rstrip())
        process.wait()
        if process.returncode == 0:
            subprocess.run(['sudo', 'usermod', '-aG', 'docker', os.getenv('USER', 'user')], capture_output=True)
